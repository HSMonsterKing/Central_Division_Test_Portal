<%@ Page Language="VB" MasterPageFile="./MasterPage.master" MaintainScrollPositionOnPostback="true" AutoEventWireup="false" CodeFile="報表作業.aspx.vb" Inherits="報表作業"%>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server" >
    ...
    <script src='js/報表作業.js'></script>
    <link rel="stylesheet" runat="server" media="screen" href="css\報表作業.css"/>
    <div><h1><a id="Title" href="報表作業.aspx">平面圖及水電管路圖<a></h1></div>
        ...
        <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="false" CellPadding="1" HorizontalAlign="Center" DataSourceID="SqlDataSource1" DataKeyNames="id" GridLines="None" ShowHeaderWhenEmpty="false" AllowPaging="True" PageSize="15" AllowSorting="True" PagerSettings-PageButtonCount=25 EnableModelValidation="True" AutoGenerateEditButton="false">
            <Columns>
                ...
                <asp:TemplateField HeaderText="檔名">
                    <ItemTemplate>
                        <asp:HyperLink ID="檔名" runat="server" NavigateUrl='<%# If(Eval("檔名").ToString = "", "", "data/報表作業檔案/" & Eval("檔名")) %>' Text='<%# Eval("檔名") %>' CssClass="檔名"></asp:HyperLink>
                    </ItemTemplate>
                    <HeaderStyle HorizontalAlign="Center"/>
                    <ItemStyle HorizontalAlign="Center" CssClass="Item"/>
                </asp:TemplateField>
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString='<%$ ConnectionStrings:ApplicationServices%>'
        ...
        Insertcommand=""
        UpdateCommand=""
        DeleteCommand="DELETE FROM 報表作業上傳 WHERE id=@id">
        <SelectParameters>
        </SelectParameters>
        <InsertParameters>
        </InsertParameters>
