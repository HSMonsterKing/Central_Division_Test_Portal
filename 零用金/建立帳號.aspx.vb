    ...
    protected Sub 建立帳號_Click(ByVal sender As Object, ByVal e As System.EventArgs)'密碼錯誤，會顯示錯誤
        If (帳號.Text<>"" And 密碼.Text<>"" And 類別.Text<>"" And 密碼.Text = 確認密碼.Text)
            ...
            If not(data_dv is Nothing) AND (data_dv.count=0)
                sql  = "SELECT type,atype FROM [帳號] WHERE type='" & 類別.Text & "'"
                data.SelectCommand = sql
                Dim data_dv2 As Data.DataView
                data_dv2 = data.Select(New DataSourceSelectArguments)
                Dim type As int32 = data_dv2(0)("type").ToString()
                Dim atype As String = data_dv2(0)("atype").ToString()
                data.InsertCommand = "Insert Into [帳號] (帳號,密碼,姓名,type,atype) Values (N'" & 帳號.Text & "',N'" & Psd() & "',N'" & 姓名.Text & "',N'" & type &"',N'" & atype &"')"
                data.Insert()
                Label1.Text = "新增成功"
                ' 設定 session 變數
                ' Session["名稱"] = 值
