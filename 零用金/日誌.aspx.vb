...
Partial Class 日誌
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            等帳_月1.SelectedValue=""
            等帳_月2.SelectedValue=""
            Me.GridView1.PageIndex = 0'Int32.MaxValue'最大數'0=第一頁
            ...
            科目.Items.Add("")
            科目.Items(0).Value = ""
            For j = 1 To data_dv.Count
                Dim 科目名稱 As String = data_dv(j-1)(0)
                科目.Items.Add(科目名稱)
                科目.Items(j).Value = 科目名稱
            Next
        Else
            Me.Label1.Text = ""
            Me.Label2.Text = ""
    ...
    Protected Sub GridView1_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridView1.RowCommand
        If e.CommandName = "查詢"
            ...
            dim 命令 As string =CType(Me.GridView1.Rows(i).FindControl("命令"), Label).text
            ...
            if left(命令,2)="刪除"
                ...
                Panel5.Visible=true
                data.ConnectionString = con_14
                data.SelectCommand = "SELECT * FROM 刪除資料表 where id=" & idm
                data_dv = data.Select(New DataSourceSelectArguments)
                For i = 0 To data_dv.Count - 1
                    D_id.text = data_dv(i)("id_收").ToString()
                    D_單位別.text = data_dv(i)("單位別").ToString()
                    D_承辦人.text = data_dv(i)("承辦人").ToString()
                    D_月.text = data_dv(i)("月").ToString()
                    D_日.text = data_dv(i)("日").ToString()
                    D_科目.text = data_dv(i)("科目").ToString()
                    D_摘要.text = data_dv(i)("摘要").ToString()
                    D_姓名.ImageUrl = data_dv(i)("姓名").ToString()
                    D_商號.text = data_dv(i)("商號").ToString()
                    D_經手人.ImageUrl = data_dv(i)("經手人").ToString()
                    D_種類.text = data_dv(i)("種類").ToString()
                    D_號數.text = data_dv(i)("號數").ToString()
                    D_收入.text = data_dv(i)("收入").ToString()
                    D_支出.text = data_dv(i)("支出").ToString()
                    D_備註.text = data_dv(i)("備註").ToString()
                    D_刪除日期.text = data_dv(i)("date").ToString()
                next
            elseIf left(命令,2)="修改"
                Panel3.Visible=False
                Panel4.Visible=true
                ...
                End If
            End If 
        ElseIf e.CommandName = "流程"'顯示該筆資料的日誌
            '取ID
            Dim i As Long = e.CommandSource.NamingContainer.RowIndex
            ...
            單位別.SelectedValue=""
            承辦人.SelectedValue=""
            月1.SelectedValue=""'改空白
            月1_SelectedIndexChanged(sender,e)
            日1.SelectedValue=(DateTime.Now.Day).ToString()
            月2.SelectedValue=(DateTime.Now.Month).ToString()
            月2_SelectedIndexChanged(sender,e)
            日2.SelectedValue=(DateTime.Now.Day).ToString()
            科目.SelectedValue=""
        ...
        End If 
    End Sub
    Protected Sub 返回主畫面_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Me.ID.text =""
        返回主畫面.visible=False
    ...
    Protected Sub 年_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles 年.TextChanged
        月1_SelectedIndexChanged(sender,e)
        月2_SelectedIndexChanged(sender,e)
        等帳_月1_SelectedIndexChanged(sender,e)
        等帳_月2_SelectedIndexChanged(sender,e)
    End Sub
    ...
    Protected Sub 月2_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles 月2.SelectedIndexChanged
        GetDay(月2,日2)
    End Sub
    Protected Sub 等帳_月1_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles 等帳_月1.SelectedIndexChanged
       GetDay(等帳_月1,等帳_日1)
    End Sub
    Protected Sub 等帳_月2_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles 等帳_月2.SelectedIndexChanged
        GetDay(等帳_月2,等帳_日2)
    End sub
