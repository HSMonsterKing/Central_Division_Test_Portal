...
Partial Class 大宗郵件執據
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            data.SelectCommand = "SELECT 掛號類別 FROM  大宗郵件執據_郵寄種類 where 序號='" & Me.DropDownList5.SelectedValue & " ' "
            data.DataBind()
            data_dv = data.Select(New DataSourceSelectArguments)
            Me.TextBox24.Text = data_dv(0)(0).ToString()
            Me.CheckBox4.Checked = False
            data.SelectCommand = "SELECT 自動清空文號 FROM  大宗郵件執據_操作者 WHERE 帳號='" & Me.TextBox2.Text & "'"
            data.DataBind()
            data_dv = data.Select(New DataSourceSelectArguments)
            If IsDBNull(data_dv(0)(0))
                data.UpdateCommand = "update 大宗郵件執據_操作者 set 自動清空文號 = 0 WHERE 帳號='" & Me.TextBox2.Text & "'"
                data.Update()
                data.DataBind()
            Else
                Me.CheckBox30.Checked = data_dv(0)(0)
            End If
        End If
    End Sub
    Private Sub CheckBox30_CheckedChanged(sender As Object, e As EventArgs) Handles CheckBox30.CheckedChanged
        data.ConnectionString = con_14
        data.UpdateCommand = "update 大宗郵件執據_操作者 set 自動清空文號 = '" & CheckBox30.Checked & "' WHERE 帳號='" & Me.TextBox2.Text & "'"
        data.Update()
        data.DataBind()
    End Sub
    ...
    Protected Sub Button2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        Dim _收費小組 As Long = 0
        data.SelectCommand = "SELECT int_data2 FROM CREATE_DOC " & _
        "INNER JOIN  COM_DATA ON CREATE_DOC.CREATE_MAN = COM_DATA.ID " & _
        "INNER JOIN  USERS ON COM_DATA.STR_DATA2 = USERS.EMP_NO " & _
        "INNER JOIN  DEPT ON USERS.DEPT_ID = DEPT.ID  " & _
        "where (int_data2=5788 or int_data2=11286) and FULL_NO='" & Me.TextBox1.Text & "'"
        data.DataBind()
        data_dv = data.Select(New DataSourceSelectArguments)
        ...
        ' If data_dv.Count = 0 Then
        '     data.SelectCommand ="SELECT id,SUBJECT FROM RECV_DOC where RECV_NO ='" & Me.TextBox1.Text & "'"
        ' End If
        data.DataBind()
        data_dv = data.Select(New DataSourceSelectArguments)
        Dim _id As Long = 0
        ...
        If data_dv.Count > 0 Then
            ...
            data.SelectCommand = "SELECT DISTINCT ACC_NAME,ADDR,ADDR_CODE,TEMP_DOC_ID,DELI_WAY FROM  ACCEPTER where  DOC_ID ='" & _id & "'"
            'data.SelectCommand = "SELECT ACC_NAME,ADDR,ADDR_CODE,TEMP_DOC_ID,DELI_WAY FROM  ACCEPTER where  DOC_ID ='" & _id & "' and DOCNAME = 'DOC1' ORDER BY ID"
            data.DataBind()
            data_dv = data.Select(New DataSourceSelectArguments)
            ''增加部份開始
            _TEMP_DOC_ID = 0
            ...
            If data_dv(i)(10).ToString() <> 0 Then
                ...
                Else
                    ...
                    Try
                        ...
                        Else
                            Me.Label2.Text = Me.Label2.Text + vbCrLf + _收件人 + " 因收件人相同 併入序號" + data_dv3(0)(0).toString
                        End If
                    Catch
                    End Try
                End If
    ...
    Protected Sub Button8_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button8.Click
        ...
        Me.Label2.Text = ""
        Dim _序號 As Long
        data.SelectCommand = "SELECT 序號 FROM  大宗郵件執據 where 年='" & Me.DropDownList1.SelectedValue & " ' and 月='" & Me.DropDownList2.SelectedValue & " ' and 日='" & Me.DropDownList3.SelectedValue & "' and 批號='" & Me.DropDownList7.SelectedValue & "' and 郵寄種類='" & Me.DropDownList5.SelectedValue & "' ORDER BY 序號 DESC"
        data.DataBind()
        data_dv = data.Select(New DataSourceSelectArguments)
        If data_dv.Count > 0 Then
            _序號 = data_dv(0)(0).ToString() + 1
        ...
        data.ConnectionString = con_14
        data.SelectCommand = "SELECT 郵資 FROM 大宗郵件執據_資費表 WHERE 序號='" & Me.DropDownList5.SelectedValue &"' ORDER BY 郵資"
        data.DataBind()
        data_dv1 = data.Select(New DataSourceSelectArguments)
        Dim _郵資 As String = Me.TextBox13.Text
        If data_dv1.Count > 1
            If Trim(data_dv1(0)(0).toString) = "0"
                _郵資 = data_dv1(1)(0).toString
            Else
                _郵資 = data_dv1(0)(0).toString
            End If
        ElseIf data_dv1.Count = 1
            _郵資 = data_dv1(0)(0).toString
        End If

        Dim _id As Long = 0
        Dim _收件人 As String = Trim(Me.TextBox14.Text)
        Dim _地址 As String = Trim(Me.TextBox15.Text)
        Dim _文號 As String = Trim(Me.TextBox17.Text)
        Dim _備註 As String = Trim(Me.TextBox16.Text)
        Dim _重量 As String = Trim(Me.TextBox12.Text)
        Dim _郵寄種類 As Long = Me.DropDownList5.SelectedValue
        Dim _郵遞區號 As String = ""
        Dim _掛號類別 As String = ""
        Dim a As String = ""
        Dim b As String = ""
    ...
    Protected Sub Button9_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button9.Click
        ...
        If data_dv.Count > 0 Then
            ...
            Me.TextBox3.Text = "主旨：" + Trim(data_dv(0)(1).ToString())
            _id = data_dv(0)(0).ToString()
            data.SelectCommand = "SELECT DISTINCT ACC_NAME,ADDR,ADDR_CODE FROM  ACCEPTER where DELI_WAY=1  and DOC_ID ='" & _id & "'"
            'data.SelectCommand = "SELECT ACC_NAME,ADDR,ADDR_CODE FROM  ACCEPTER where DELI_WAY=1  and DOC_ID ='" & _id & "' and DOCNAME = 'DOC1' ORDER BY ID"
            data.DataBind()
            data_dv = data.Select(New DataSourceSelectArguments)
            data.ConnectionString = con_14
            Dim _郵寄種類 As Long = 1
    ...
    Protected Sub Button12_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button12.Click
        ...
        If data_dv.Count > 0 Then
            ...
            Me.TextBox3.Text = "主旨：" + Trim(data_dv(0)(1).ToString())
            _id = data_dv(0)(0).ToString()
            data.SelectCommand = "SELECT DISTINCT ACC_NAME,ADDR,ADDR_CODE FROM  ACCEPTER where DELI_WAY=3  and DOC_ID ='" & _id & "'"
            'data.SelectCommand = "SELECT ACC_NAME,ADDR,ADDR_CODE FROM  ACCEPTER where DELI_WAY=3  and DOC_ID ='" & _id & "' and DOCNAME = 'DOC1' ORDER BY ID"
            data.DataBind()
            data_dv = data.Select(New DataSourceSelectArguments)
            data.ConnectionString = con_14
            Dim _郵寄種類 As Long = 1
    ...
    Public Sub addresshandler(ByRef _郵遞區號 As String, ByRef _地址 As String)
        _郵遞區號 = Trim(_郵遞區號)
        _地址 = Trim(_地址)
        Dim regex1 as new Regex("^[0-9]{3,6}[^0-9]+")
        Dim regex2 as new Regex("^[0-9]{3,6}")
        Dim regex3 as new Regex("^[0-9]{3,6}$")
        Dim temp As String = ""
        If regex1.IsMatch(_郵遞區號)
            _地址 = Trim((regex2.Split(_郵遞區號))(1))
            _郵遞區號 = (regex2.Match(_郵遞區號)).ToString()
        End If
        _郵遞區號 = Trim(_郵遞區號)
        _地址 = Trim(_地址)
        If regex1.IsMatch(_地址)
            _郵遞區號 = (regex2.Match(_地址)).ToString()
            _地址 = Trim((regex2.Split(_地址))(1))
        End If
        _郵遞區號 = Trim(_郵遞區號)
        _地址 = Trim(_地址)
        If regex3.IsMatch(_地址)
            temp = _地址
            _地址 = _郵遞區號
            _郵遞區號 = temp
        End If
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Dim Excel As String = MapPath(".\Excel\Temp\") & _GUID & ".xls"
        If FileUpload1.HasFile Then
            Dim old As String = MapPath(".\Excel\Temp\") & FileUpload1.FileName
            FileUpload1.SaveAs(old)
            Dim xlApp As New Excel.ApplicationClass()
            xlApp.DisplayAlerts = False
            xlApp.ScreenUpdating = false
            xlApp.EnableEvents = false
            Dim xlWorkBook As Excel.Workbook
            xlWorkBook = xlApp.Workbooks.Open(old, 0, False)
            xlWorkBook.SaveAs(Excel, FileFormat:=xlWorkbookNormal)
        ...
        End If
        '匯入
        If System.IO.File.Exists(Excel) Then
            Dim xlApp As New Excel.ApplicationClass()
            xlApp.DisplayAlerts = False
            xlApp.ScreenUpdating = false
            xlApp.EnableEvents = false
            ...
            xlWorkBook = xlApp.Workbooks.Open(Excel, 0, False, 5, "", "", True, Microsoft.Office.Interop.Excel.XlPlatform.xlWindows, "\t", False, False, 0, True, 1, 0)
            ...
            End If
            If _郵資 <> 0 Then
                data.SelectCommand = "SELECT 重量 FROM  大宗郵件執據_資費表 where 郵資='" & _郵資 & "' and 序號='" & Me.DropDownList5.SelectedValue & "' "
                data.DataBind()
                data_dv = data.Select(New DataSourceSelectArguments)
                If data_dv.Count > 0 Then
                    _重量 = data_dv(0)(0).ToString()
            ...
            Else
                If _重量 <> 0 Then
                    data.SelectCommand = "SELECT 郵資 FROM  大宗郵件執據_資費表 where 重量='" & _重量 & "' and 序號='" & Me.DropDownList5.SelectedValue & "' "
                    data.DataBind()
                    data_dv = data.Select(New DataSourceSelectArguments)
                    If data_dv.Count > 0 Then
                        _郵資 = data_dv(0)(0).ToString()
            ...
            For i As Long = 2 To 100000
                _收件人 = Trim(xlWorkSheet.Cells(i, 1).value)
                _郵遞區號 = Trim(xlWorkSheet.Cells(i, 2).value)
                _地址 = Trim(xlWorkSheet.Cells(i, 3).value)
                
                addresshandler(_郵遞區號, _地址)
                
                If Len(_收件人) <= 0 Then
                    Exit For
                End If
                'data.InsertCommand = "INSERT INTO 大宗郵件執據(年,月,日,批號,序號,收件人,地址,文號,郵遞區號,備註,重量,郵資,郵寄種類,掛號類別,件數,收費小組) VALUES ('" & Me.DropDownList1.SelectedValue & "','" & Me.DropDownList2.SelectedValue & "','" & Me.DropDownList3.SelectedValue & "','" & Me.DropDownList7.SelectedValue & "','" & _序號 & "',NULLIF(N'" & _收件人 & "',''),NULLIF(N'" & _地址 & "',''),NULLIF('" & _文號 & "',''),NULLIF('" & _郵遞區號 & "',''),NULLIF(N'" & Trim(Me.TextBox16.Text) & "',''),'" & _重量 & "','" & _郵資 & "','" & Me.DropDownList5.SelectedValue & "',NULLIF('" & Me.TextBox24.Text & "',''),'" & _件數 & "','" & _收費小組 & "')"
                'data.Insert()
                'data.DataBind()
            ...
            Me.GridView2.DataBind()
        Else
            Response.Write("<script language=javascript>alert('查無檔案請確定是否匯入檔案!')</script>")
        End If
    End Sub
End Class
